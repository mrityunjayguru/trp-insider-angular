...
@Component({
  ...
  styleUrls: ['./licencetype.component.css']
})
...
export class LicencetypeComponent {
  ...
  constructor(logService : LogService ,formBuilder: FormBuilder,   apiService: ApiService){
    ...
    this.formsize = this.formBuilder.group({
      licencetypename:['',Validators.required],
      

    });
      

    this.apiService.getAllLicenseType().subscribe(
      (response : any) => {
               
        this.departments = response.data;
        console.log("getAllLicenseType");
...
formData.append('save', JSON.stringify(payload));
   ...
   this.apiService.saveLicenseType(formData).subscribe(
    (response : any) => {
