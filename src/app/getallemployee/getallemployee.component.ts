...
@Component({
  selector: 'app-getallemployee',
  templateUrl: './getallemployee.component.html',
  styleUrls: ['./getallemployee.component.css']
})
...
export class GetallemployeeComponent {
  ...
  constructor( logService : LogService ,headerFooterService :HeaderfooterService, apiService :ApiService,imgConfigService: ImgconfigService,route: ActivatedRoute,http:HttpClient)
  {
   ...
   // this.headerFooterService.enableFooter();
    ...
    this.apiService.getAllEmployee(obj).subscribe(
      (response:any) => {
  ...
  nextPage(event: any) {
   ...
   // alert(event.pageIndex + " " + event.pageSize);
    ...
    this.apiService.showEmployee(request).subscribe(
      (response:any) => {
