...
@Component({
  selector: 'app-vendor-merchant-add-product',
  templateUrl: './vendor-merchant-add-product.component.html',
  styleUrls: ['./vendor-merchant-add-product.component.css']
})
// export class VendorMerchantAddProductComponent {
...
export class VendorMerchantAddProductComponent {
  ...
  constructor(formBuilder: FormBuilder, apiService: ApiService, router: Router) {
    ...
    if (this.Vendorusers != undefined) {
      if (this.Vendorusers.roleid === "05" && this.Vendorusers.firstName.length > 0) {
        // alert(" valid Vendor Marchant ")
      }
      else {
        alert(" Not a Vaild vendor : Need to login with vendor credential ");
    ...
    this.productsForm = this.formBuilder.group({
      ...
      wsp: [0.0],
      description: [''],
      vendorname: [{ value: this.Vendorusers.firstName, disabled: false }],
      imgfile0: [''],
      imgfile1: [''],
      imgfile2: [''],
      imgfile3: [''],
  ...
  onSubmit() {
    ...
    this.apiService.saveProduct(this.formData).subscribe(
      (response: any) => {
        alert(response.mesage);
        if (response.mesage == "Data Stored successfully." || response.status == 200) {
          this.router.navigateByUrl('/vendor-merchant-add-product.component');
        }
      })
  }
