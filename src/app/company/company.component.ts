...
export class CompanyComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.productsForm.value.companyName.length == 0) {
      alert("Kinldy enter the company name.");
      return;
    }
    ...
    else if (this.productsForm.value.companyType.length == 0) {
      alert("Kinldy enter  the company type ");
      return;
    }
    else if (this.productsForm.value.status.length == 0) {
      alert("Kinldy enter  the status name ");
      return;
    }

    else if (this.productsForm.value.emailId.length == 0) {
      alert("Kinldy enter  the email");
      return;
    }
    else if (this.productsForm.value.mobileNumber.length == 0 || this.productsForm.value.mobileNumber == 0) {
      alert("Kinldy enter  the mobile number");
      return;
    }
    else if (this.productsForm.value.gstNumber.length == 0 || this.productsForm.value.gstNumber == 0) {
      alert("Kinldy Enter  the GST number");
      return;
    }
    else {
    ...
    this.apiService.saveCompany(this.formData).subscribe(
      (response: any) => {

        alert(response.mesage);
        if (response.mesage == "Data Stored successfully." || response.status == 200) {

          this.router.navigateByUrl('/company.component');
        }
        else
        {
